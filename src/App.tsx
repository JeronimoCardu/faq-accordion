...
export default function App() {
  ...
  return (
    <>
      ...
      />
      <section className="tablet:max-w-8/10 desktop:w-4/10 tablet:p-[2.5rem] z-100 mx-auto max-h-screen w-9/10 overflow-y-auto rounded-[.5rem] bg-white p-[1.5rem]">
        <Logo />
        {faqs.map((q, index) => (
          <>
            <Accordion key={index} question={q.question} answer={q.answer} />
            {index != faqs.length - 1 && <hr className="text-purple-100" />}
          </>
        ))}
      </section>
    </>
